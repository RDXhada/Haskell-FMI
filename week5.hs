...
-- filter finds the number bigger/smaller than the given on the array
filter1 :: (a -> Bool) -> [a] -> [a]
filter1 _ [] = []
filter1 f (x:xs)
...
--func to output +1 on a given number 5 -> 6
plus1 :: Num a => a -> a
plus1 x = x+1

--Lambda function/ламбда функция(анонимна функция)
plus2 = \x -> x+2

--map1 plus1 [1,2] gives [2,3]
map1 :: (t -> a) -> [t] -> [a]
map1 _ [] = []
...
--Lambda function
--output only the odd numbers on the list and double x them (*2)
sol1 l = map1 (\x -> 2*x) $ filter1 odd l


--List comprehension (combines filter and map)
sol2 l = [ 2*x | x <- l, odd x, x < 7 && x > 3]
...
--Tuple
sol3 :: (Num c, Eq c, Enum c) => c -> [(c, c, c)]
...
getYearInformation :: (Eq a1, Eq a2) => [(a2, a3, b, a1)] -> a2 -> a1 -> [(a3, b)]
getYearInformation l cityNameParam yearParam = [(temperature, month) | (cityName, temperature, month, year) <- l, yearParam == year, cityNameParam == cityName]
